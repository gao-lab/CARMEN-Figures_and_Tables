...
filter_n=int(sys.argv[3])
filter_l=int(sys.argv[4])
pool_l=int(sys.argv[5])
...
def main():
    ...
    for fdn in [0,1,2,3,4]:
        ...
        training(x_train, y_train, x_test, y_test,fold_num=fdn,sd=random_seed,batch_size=200, number_of_epoch=50,filter_num=filter_n,lr=0.01,units=512,filter_len=filter_l,pool_len=pool_l)
