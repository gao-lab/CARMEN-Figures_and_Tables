...
infile <- read.table("./FigS11/deepsea_down_sample_AUROC.txt",sep='\t',stringsAsFactors = F, header = T)
...
ggsave("./FigS11/DeepSEA-downsampling-AUC-compare.pdf")

#=========================AUCPRC============================================

infile <- read.table("./FigS11/deepsea_down_sample_AUPRC.txt",sep='\t',stringsAsFactors = F, header = T)
colnames(infile) <- c("Assay Categories","CARMEN Annotations","DeepSEA","DeepSEA Downsampling")
infilep <-melt(infile) 
...
ggsave("./FigS11/DeepSEA-downsampling-AUPRC-compare.pdf")
