...
clf = joblib.load('RF_1200_1KG-HGMDDM-sampleweight_estimators4.pkl')
feature_importance_score = pd.DataFrame(clf.feature_importances_)
feature_list = pd.read_table('1190_feature_list.txt',sep=' ',header=None)
feature_importance_score = pd.DataFrame(clf.feature_importances_)
...
grouped = features.iloc[:,3].groupby(features.iloc[:,2])
g_rank = np.array(grouped.rank()) # calculate the rank in each group
g_num = np.array(grouped.count()[features.iloc[:,2]]) # count the number of each group
g_ratio = pd.DataFrame(g_rank / g_num)
f_ratio = features.iloc[:,3].rank() / float(features.shape[0]) # the feature importance in all features
...
features.to_csv('features_1190.odds.csv',index=None)
